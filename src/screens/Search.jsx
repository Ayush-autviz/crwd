...
const SearchScreen = () => {
  ...
  return (
    <div >
        ...
        <div className="text-2xl   text-center md:text-5xl   font-bold text-[#393939] mb-5">Find CRWD and Nonprofits</div>
...
/>
        ...
        <button onClick={()=>setType('crwd')}       className={`
        px-8 flex-1 py-3 
        text-[14px]
        text-nowrap
        md:text-xl
    ...
    >
        ...
        <button onClick={()=>setType('nonprofits')}       className={`
        px-8 flex-1 py-3 
        text-[14px]
        md:text-xl
        rounded-full border 
    ...
    >
         ...
         <div className='flex flex-col w-full   gap-3 mt-10 md:mt-15'>
           <div className="text-2xl   md:text-5xl  font-bold text-[#393939] ">CRWDs</div>
           <div className="text-sm   md:text-lg  font-light text-[#393939] w-[85%] md:w-[60%] "> Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae.</div>
         </div>
         ...
         <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {
            crwds.map((item,index)=>{
              return   <NonProfitSearch key={index} item={item}/>
            })
        }
      </div>
