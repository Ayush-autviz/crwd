...
const NonProfitSearch = ({item}) => {
  ...
  return (
    <div className='rounded-xl p-4 flex flex-col xl:flex-row   gap-5  bg-[#f3f2f2]'>
        <img src={item.image} className='rounded-xl w-full h-[200px]    xl:w-[304px] xl:h-[260px] object-right-top object-cover '/>
        <div className='flex flex-col gap-2  items-start'>
        <div className="space-y-2">
            <h3 className="text-xl md:text-2xl font-semibold text-gray-900">{item.title}</h3>
            <p className="text-[12px] md:text-[14px]  text-gray-600">{item.desc}</p>
        </div>
        ...
        <button onClick={navigateNonprofit} className="text-[12px] md:text-[14px] px-4 py-2 md:px-8 md:py-2 bg-[#393939] hover:shadow-xl transition-shadow cursor-pointer rounded-full border border-gray-300 text-white font-medium">
          Donate Now
        </button>
